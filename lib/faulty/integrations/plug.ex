defmodule Faulty.Integrations.Plug do
  @moduledoc """
  ...
  * `request.headers`: headers received on the request. All headers are included
  ...
  def report_error(conn, reason, stack) do
    unless Process.get(:faulty_router_exception_reported) do
      try do
        Faulty.report(reason, stack, build_context(conn))
      after
        Process.put(:faulty_router_exception_reported, true)
      end
    end
  end
