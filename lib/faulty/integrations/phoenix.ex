defmodule Faulty.Integrations.Phoenix do
  @moduledoc """
  ...
  `Faulty.Integrations.Plug` integration too.
  ...
  can find on the `Faulty.Integrations.Plug` integration.
  ...
  * `live_view.event_params`: last event params received by the LiveView
  ...
  def handle_event([:phoenix, :live_view, :handle_event, :exception], _, metadata, :no_config) do
    Faulty.report({metadata.kind, metadata.reason}, metadata.stacktrace, %{
